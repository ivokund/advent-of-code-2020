...
const realInput = `wnwwswnewewnwwnesewseewnesewnwsenw
...
const getDirDelta = ([curX, curY], direction) => {
    const hasOffset = curY % 2 === 0

    switch (direction) {
        case 'w': return [-1, 0];
        case 'e': return [1, 0];
        case 'nw': return [hasOffset ? 0 : -1, -1];
        case 'ne': return [hasOffset ? 1 : 0, -1];
        case 'sw': return [hasOffset ? 0 : -1, 1];
        case 'se': return [hasOffset ? 1 : 0, 1];
    }
}

function part1(input) {
    const instructions = input.split("\n").map((line) => {
        return line.split(/(se|sw|nw|ne|e|w)/).filter(Boolean)
    })

    const flipped = {}

    let flippedOnceCount = 0

    instructions.forEach((instructions) => {
        const coords = [0, 0]

        instructions.forEach((direction) => {
            const [dx, dy] = getDirDelta(coords, direction)
            coords[0] += dx
            coords[1] += dy
        })
        ...
        if (flipped[coordsStr]) {
            delete flipped[coordsStr]
        } else {
            flipped[coordsStr] = true
            flippedOnceCount++
        }

    })
    console.log('black tiles: ' + Object.keys(flipped).length)
    return flipped
}
...
function part2(input, days) {
    ...
    const blackTileCoords = part1(input)

    const getNeighbours = ([x, y]) => {
        const deltas = ['w', 'e', 'nw', 'ne', 'sw', 'se']
        const coords = deltas.map((direction) => getDirDelta([x, y], direction))
        return coords.map(([dx, dy]) => [x + dx, y + dy])
    }

    for (let day=0; day < days; day++) {
        const toFlipWhite = {}
        const toFlipBlack = {}
        const allWhiteNeighbours = {}

        Object.keys(blackTileCoords).forEach((coordStr) => {
            const coords = coordStr.split(',').map(Number)
            const allNeighbours = getNeighbours(coords)
            const blackNeighbourCount = allNeighbours
                .filter((xy) => blackTileCoords[xy.join(',')])
                .length

            allNeighbours
                .filter((xy) => !blackTileCoords[xy.join(',')])
                .forEach((xy) => allWhiteNeighbours[xy.join(',')] = true)

            if (blackNeighbourCount === 0 || blackNeighbourCount > 2) {
                toFlipWhite[coordStr] = true
            }
        })

        Object.keys(allWhiteNeighbours).forEach((coordStr) => {
            const coords = coordStr.split(',').map(Number)
            const allNeighbours = getNeighbours(coords)
            const blackNeighbourCount = allNeighbours
                .filter((xy) => blackTileCoords[xy.join(',')])
            ...
            if (blackNeighbourCount === 2) {
                toFlipBlack[coordStr] = true
            }
        })

        Object.keys(toFlipBlack).forEach((coordStr) => {
            blackTileCoords[coordStr] = true
        })
        Object.keys(toFlipWhite).forEach((coordStr) => {
            delete blackTileCoords[coordStr]
        })
    }

    console.log('black tile count:' + Object.keys(blackTileCoords).length)
}
