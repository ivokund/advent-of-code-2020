...
let count2 = 0;
input.forEach((line) => {
   const a = line.match(/(\d+)\-(\d+)\s(\w):\s(\w+)/) ;
   const [,from, to, letter, password] = a;
   const regex = new RegExp(`${letter}`, 'g');
   const result = (password.match(regex) || []).length
   console.log({ password, regex, letter, result });
   ...
   const hasFirst = parts[from-1] === letter;
   const hasSecond = parts[to-1] === letter;
   count2 += (hasFirst || hasSecond) && (!hasFirst || !hasSecond)
});
console.log(count2);