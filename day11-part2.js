...
const realInput = `LLLLL.LLLLLLLLL.LLLLLLLLLL.LLLLLLL.LLLLLLLLLLLLLLLLL.LLLLLLLLL.LLLLLL.LLLLLLL.LLLLLLLLLLLLLLLLLLLL
...
const getVisibleSeatCount = (layout, x, y) => {
    const directions = [
        [-1, -1],
        [-1, 0],
        [-1, 1],
    ...
    const counts = directions.map(([dx, dy]) => {
        ...
        do {
            newX = newX + dx;
            newY = newY + dy;
            const val = layout?.[newX]?.[newY];
            if (!val || val === 'L') { // out of bounds
                return 0;
            } else if (val === '#') {
                return 1;
            } else {
...
function countOccupied (layout) {
    return layout.reduce((sum, row) => {
        return sum + row.filter((seat) => seat === '#').length;
    }, 0)
}



function evolveStep (prevLayout) {

    let numChanged = 0;
    const newLayout = prevLayout.map((row, rowNo) => {
        return row.map((seat, colNo) => {
            const count = getVisibleSeatCount(prevLayout, rowNo, colNo);
            if (seat === 'L' && count === 0) {
                // console.log(`changing ${rowNo}x${colNo} to '#' because rule 1`)
                numChanged++;
                return '#';
            } else if (seat === '#' && count >= 5) {
                // console.log(`changing ${rowNo}x${colNo} to 'L' because rule 2`)
                numChanged++;
                return 'L';
            } else {
                // console.log(`keeping ${rowNo}x${colNo} the same`)
                return seat;
            }
        });
    });
    return { numChanged, newLayout };
}
...
function part2 () {
    ...
    while (numOfChanges > 0) {
        ...
        const { numChanged, newLayout } = evolveStep([...layout])
        console.log(print(newLayout), { numChanged });
        numChangedLastStep = numChanged;
        numOfChanges += numChanged;
        layout = newLayout;

        if (numChanged === 0) {
            console.log('num of seated: ' + countOccupied(layout));
            break;
        }
        iteration++;
    }
