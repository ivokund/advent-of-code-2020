...
function part1(input) {
    ...
    const foods = input.split(`\n`).map((line) => {
        const [,ingredients, allergens] = line.match(/^([\w\s]+) \(contains ([\w\s,]+)\)$/);
        return [ingredients.split(' '), allergens.split(', ')];
    })

    const allIngredients = Object.keys(foods.reduce((acc, [ingredients]) => {
        ingredients.forEach((name) => {
            acc[name] = true;
        })
        return acc;
    }, {}))
    const ingredientCounts = foods.reduce((acc, [ingredients]) => {
        ingredients.forEach((name) => {
    ...
    const allergensByIngredient = foods.reduce((acc, [ingredients, allergens]) => {
        ingredients.forEach((ingredient) => {
            if (!acc.has(ingredient)) {
                acc.set(ingredient, new Set());
            }
            acc.get(ingredient).add(...allergens);

        })
        return acc;
    }, new Map())

    const ingredientsByAllergen = foods.reduce((acc, [ingredients, allergens]) => {
        allergens.forEach((allergen) => {
            if (!acc.has(allergen)) {
                acc.set(allergen, new Set(ingredients));
            } else {
                acc.set(allergen, new Set(ingredients.filter((i) => acc.get(allergen).has(i))));
            }
            // acc.get(name).add(...ingredients);
        })
        return acc;
    }, new Map())


    const remainingIngredients = new Set(allIngredients);

    const map = {};
    for (let [i, aSet] of allergensByIngredient) {
        [...aSet].forEach((allergen) => {
            const ingByAllergen = ingredientsByAllergen.get(allergen)
            if (ingByAllergen.has(i)) {
                if (!map[allergen]) map[allergen] = [];
                map[allergen].push(i)
                remainingIngredients.delete(i)
            }
        })
    }

    const count = [...remainingIngredients].reduce((sum, ingredient) => {
        sum += ingredientCounts[ingredient];
        return sum;
    }, 0)
    ...
    while (Object.keys(stack).length > 0) {
        console.log('-----')
        const [a, [i]] = Object.entries(stack).find(([a, i]) => i.length === 1);

        finalMap[a] = i;
        console.log({finalMap, a, i })

        const newStack = Object.entries(stack).map(([a, ingList]) => {
            return [
                a,
                ingList.filter((name) => name !== i),
            ]
        }).filter(([a, ingList]) => ingList.length > 0)

        console.log(newStack)
        //

        stack = Object.fromEntries(newStack);
    }

    console.log(finalMap)

    const sorted = Object.keys(finalMap).sort().reduce((acc, item) => {
        acc[item] = finalMap[item];
        return acc;
    }, {})

    console.log({ sorted })
    const answer = Object.values(sorted).join(',');

    console.log({answer})
    // fdsfpg,jmvxx,pqqks,pqqks,kfgln,pqqks,pqrvc,pqrvc
}
