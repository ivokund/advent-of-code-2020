...
const rules2 = rules.reduce((rulesByBag, rule) => {
    const [, index, contentsCsv] = rule.match(/^(\w+\s\w+) bags? contain (.*)/);
    rulesByBag[index] = contentsCsv.split(', ').map((item) => {
        const match = item.match(/^(no other|(\d+)\s(\w+\s\w+)) bags?s?/);
...
const winningBags = new Set();

while (stack.length > 0) {
    const item = stack.pop();
    Object.entries(rules2).forEach(([k,v]) => {
        if (v.includes(item)) {
            stack.push(k);
            winningBags.add(k);
        }
...
console.log(winningBags.size);


/// part 2

const rules3 = rules.reduce((rulesByBag, rule) => {
    const [, index, contentsCsv] = rule.match(/^(\w+\s\w+) bags? contain (.*)/);
    rulesByBag[index] = contentsCsv.split(', ').map((item) => {
        const match = item.match(/^(no other|(\d+)\s(\w+\s\w+)) bags?s?/);
        if (match[0] === 'no other bags') {
...
let bags = 0;
const getSumRec = (color, mult = 1) => {
    rules3[color].forEach(({ count, itemName }) => {
        const countNum = parseInt(count, 10);
        bags += countNum * mult;
        getSumRec(itemName, countNum * mult);
    });
};
