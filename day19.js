...
const messages = `bbbbbbbbbaaaabbaababbabbaaabbbabbbbaaabb
...
4: "a"
5: "b"`;
const testMessages = `ababbb
bababa
abbbab
aaabbb
aaaabbb`;

const testRules2 = `42: 9 14 | 10 1
9: 14 27 | 1 26
10: 23 14 | 28 1
1: "a"
11: 42 31
...
const testMessages2 = `abbbbbabbbaaaababbaabbbbabababbbabbbbbbabaaaa
bbabbbbaabaabba
...
function part1(rulesText, messagesText) {

    const rules = rulesText.split('\n')
        .map((ruleLine) => ruleLine.split(': '));
    const rulesByNo = Object.fromEntries(rules);
    ...
    const parseRule = (rule) => {
        ...
        const a = orRules.map((orRule) => orRule.split(' ')
            .map((ruleNo) => parseRule(rulesByNo[ruleNo]))
            .join('')
        );
        return `(${a.join('|')})`;
    }

    const normalizedRules = Object.fromEntries(rules.map(([ruleNo, ruleLine]) => {
        return [ruleNo, parseRule(ruleLine)];
    }));
    const ruleRegex = `^${normalizedRules['0']}$`;
    console.log(ruleRegex);

    const msgCount = messagesText.split('\n')
        .filter((msgLine) => new RegExp(ruleRegex).test(msgLine))
        .length;
...
function part2(rulesText, messagesText) {
    rulesText = rulesText.replace('8: 42', '8: 42 | 42 8');
    rulesText = rulesText.replace('11: 42 31', '11: 42 31 | 42 11 31');
console.log(rulesText);
    const rules = rulesText.split('\n')
        .map((ruleLine) => ruleLine.split(': '));
    const rulesByNo = Object.fromEntries(rules);

    const parseRule = (ruleNo, stackCount = 1) => {
        const rule = rulesByNo[ruleNo];
        if (rule.startsWith('"')) {
            return rule.substring(1, rule.length-1);
        }
        const orRules = rule.split(' | ');
        const a = orRules.map((orRule) => orRule.split(' ')
            .map((ruleNo2) => {
                if (stackCount > 15) {
                    ...
                    return parseRule(ruleNo2, ruleNo == ruleNo2 ? stackCount+1: stackCount)
                }
            })
            .join('')
        );
    ...
    const normalizedRules = Object.fromEntries(rules.map(([ruleNo, ruleLine]) => {
        return [ruleNo, parseRule(ruleNo)];
    }));
    const ruleRegex = `^${normalizedRules['0']}$`;
    console.log(ruleRegex);
...
// part2(testRules2, testMessages2);
