...
const getOccupiedCount = (layout, row, col) => {
    const val = layout?.[row]?.[col];
    return val === '#' ? 1 : 0;
}

const getOccupiedCountAdj = (layout, row, col) => {
    return getOccupiedCount(layout, row - 1, col-1)
        + getOccupiedCount(layout, row - 1, col)
        + getOccupiedCount(layout, row - 1, col+1)
        // middle row
        + getOccupiedCount(layout, row, col-1)
        + getOccupiedCount(layout, row, col+1)
        // bottom row
        + getOccupiedCount(layout, row + 1, col-1)
        + getOccupiedCount(layout, row + 1, col)
...
function evolveStep (prevLayout) {
    ...
    const newLayout = prevLayout.map((row, rowNo) => {
        return row.map((seat, colNo) => {
            const count = getOccupiedCountAdj(prevLayout, rowNo, colNo);
            if (seat === 'L' && count === 0) {
                numChanged++;
                return '#';
