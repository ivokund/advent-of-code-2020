const input = `iyr:2013 hcl:#ceb3a1
hgt:151cm eyr:2030
byr:1943 ecl:grn
...
const isValid = (field) => {
    ...
    const matchers = {
        byr: (val) => val >= 1920 && val <= 2002 && val.match(/\d{4}/),
        iyr: (val) => val >= 2010 && val <= 2020 && val.match(/\d{4}/),
        eyr: (val) => val >= 2020 && val <= 2030 && val.match(/\d{4}/),
        hgt: (val) => val.match(/^(\d)+cm/) && val.substring(0, val.length - 2) >= 150 && val.substring(0, val.length - 2) <= 193
          || val.match(/^(\d)+in/) && val.substring(0, val.length - 2) >= 59 && val.substring(0, val.length - 2) <= 76,
        hcl: (val) => val.match(/^#[a-f0-9]{6}/),
        ecl: (val) => val.match(/amb|blu|brn|gry|grn|hzl|oth/),
        pid: (val) => val.match(/^\d{9}$/),
        cid: (val) => true,
    };
    return matchers[name](value);
};

const valid = passports.filter((passport) => {
    const fields = passport.split(/[\n|\s]/);
    const cid = fields.find((field) => field.startsWith('cid:'));

    const validFields = fields.filter(isValid);
    const invalidFields = fields.filter((f) => !isValid(f));

    return validFields.length === (cid ? 8 : 7);
});
